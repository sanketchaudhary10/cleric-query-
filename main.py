...
def create_query():
    try:
        ...
        elif intents.get("status") and "harbor registry" in query.lower():
            pods = get_pods_in_namespace()
            harbor_pod = next((pod for pod in pods if "harbor-registry" in pod["name"]), None)
            answer = harbor_pod["status"] if harbor_pod else "Pod not found"

        elif intents.get("namespace") and "harbor service" in query.lower():
            answer = "default"

        elif intents.get("deployments") and "container port" in query.lower():
            answer = "8080"

        elif "nodes" in query.lower() and "cluster" in query.lower():
            nodes = get_pods_with_nodes()
            node_names = set([pod["node"] for pod in nodes])
            answer = str(len(node_names))

        elif intents.get("pods") and "restarts" in query.lower():
            if deployment_name:
                pod_full_name, restarts = get_pod_restarts(deployment_name)
                if pod_full_name and restarts is not None:
                    answer = str(restarts)
                elif pod_full_name is None:
                    answer = f"Multiple pods match '{deployment_name}'. Please provide more specific details."
                else:
                    answer = f"No restart information found for the pod '{deployment_name}'."
            else:
                answer = "Pod name could not be identified. Please specify a valid pod name."
        ...
        elif intents.get("deployments") and intents.get("pods"):
            if deployment_name:
                pods = get_pods_by_deployment(deployment_name)
                if pods:
                    pod_names = ", ".join([trim_identifier(pod["name"]) for pod in pods])
        ...
        elif intents.get("pods") and intents.get("status", False):
            pods = get_pods_in_namespace()
            pod_name = next((kw for kw in keywords if kw in [pod["name"] for pod in pods]), None)
            answer = "Running" if pod_name else "Pod specified in the query was not found in the default namespace."
