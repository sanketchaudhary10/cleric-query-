...
def get_pod_restarts(deployment_name, namespace="default"):
    pods = get_pods_by_deployment(deployment_name, namespace)
    if len(pods) > 1:
        return None, None  # Indicating multiple matches
    elif pods:
        pod = pods[0]
        pod_details = get_pods_in_namespace(namespace=namespace)
        for details in pod_details:
            if details["name"] == pod["name"]:
                return details["name"], details["restarts"]
    return None, None
